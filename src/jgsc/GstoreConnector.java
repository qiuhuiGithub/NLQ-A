...
public class GstoreConnector {
	...
	public static int defaultServerPort = 3305;
	...
	public boolean load(String _db_name) {
		...
		if (!connect_return) {
			System.err
					.println("connect to server error. @GstoreConnector.load");
			return false;
		}
		...
		if (!send_return) {
			System.err
					.println("send load command error. @GstoreConnector.load");
			return false;
		}
		...
		System.out.println(recv_msg); // debug
	...
	public boolean unload(String _db_name) {
		...
		if (!connect_return) {
			System.err
					.println("connect to server error. @GstoreConnector.unload");
			return false;
		}
		...
		if (!send_return) {
			System.err
					.println("send unload command error. @GstoreConnector.unload");
			return false;
		}
		String recv_msg = this.recv();
	...
	public boolean build(String _db_name, String _rdf_file_path) {
		...
		if (!connect_return) {
			System.err
					.println("connect to server error. @GstoreConnector.build");
			return false;
		}
		...
		if (!send_return) {
			System.err
					.println("send import command error. @GstoreConnector.build");
			return false;
		}
		String recv_msg = this.recv();
	...
	public String query(String _sparql) {
		...
		if (!send_return) {
			System.err
					.println("send query command error. @GstoreConnector.build");
	...
	private boolean connect() {
		try {
			...
			return (this.socket != null && this.socket.isConnected() && this.socket
					.isBound());
		} catch (Exception e) {
			System.err
					.println("socket connection error. @GstoreConnector.connect");
			e.printStackTrace();
		}
	...
	private boolean disconnect() {
		try {
			...
			System.err
					.println("socket disconnection error. @GstoreConnector.disconnect");
			e.printStackTrace();
		}
	...
	private String recv() {
		try {
			...
			// in Java String, there is no need for terminator '\0' in C. so we
			// should omit '\0' at the end of receiving message.
			byte[] data_context = new byte[context_len - 1];
			int recv_len = 0;
			do {
				int cur_len = dis.read(data_context, recv_len,
						data_context.length - recv_len);
				recv_len += cur_len;
	...
	private static byte[] packageMsgData(String _msg) {
		...
		int context_len = data_context.length + 1; // 1 byte for '\0' at the end
													// of the context.
		int data_len = context_len + 4; // 4 byte for one int(data_len at the
										// data's head).
		...
		// in C, there should be '\0' as the terminator at the end of a char
		// array. so we need add '\0' at the end of sending message.
		data[data_len - 1] = 0;
